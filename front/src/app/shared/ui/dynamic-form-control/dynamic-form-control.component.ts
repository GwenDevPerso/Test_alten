...
export class DynamicFormControlComponent<T> {
  ...
  onFileUploaded(event: any): void {
    this.formCtrl.setValue(event.file);
    this.image = event.file.name;
    this.valuePreset.emit({ ctrl: this.controlConfig.key as string, value: event.file });
  }

  onFileDeleted(event: any): void {
    this.formCtrl.setValue(null);
    this.valuePreset.emit({ ctrl: this.controlConfig.key as string, value: null });
  }
}
